...
def write_png(html, target=None, base_url=None, extra_stylesheets=()):
    """Write the provided HTML in a PDF file.
    ...
    if target in [None, "@memory"]:
        with BytesIO() as buffer:
            weasy_html.write_png(buffer, stylesheets=extra_stylesheets, resolution=96*4)
            pdf_data = buffer.getvalue()
        return pdf_data
    else:
        weasy_html.write_png(target, stylesheets=extra_stylesheets, resolution=96*4)
...
class LabelWriter:
    ...
    **default_context
    ...
    def write_png_labels(self, records, target=None, extra_stylesheets=(),
                     base_url=None):
        return write_png(
            self.records_to_html(records),
            target=target,
